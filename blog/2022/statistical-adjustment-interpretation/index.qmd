...
dimensions of the data, considering 1,000, 5,000, and 10,000 observations and repeat the 
simulation 250 times per cell, resulting in 1,500 unique datasets.

To provide a breif overview of the simulation design, I begin by drawing fixed values for the 
unobserved confounders $U$ and $V$ at 1.5 and 0.5, respectively. The path coefficients 
$\gamma_{k}$ for the dependencies between nodes are then drawn from a normal distribution 
such that $\gamma_{k} \sim \mathrm{Normal(0.5, 0.5)}$ for $k \in \{1, \dots, K\}$. The 
treatment propensity $\theta$ for the exposure $X$ and the measured confounders are then each a 
function of their respective ancestor nodes and random noise $\sigma \sim \mathrm{Normal(0, 0.1)}$ 
as follows
...
where the $\delta U$ in the equation for $Z$ represents the experimental manipulation. 
...
adjustment set $\{J, W, L\}$ can be expected to exhibit severe bias due to unobserved confounding. 
...
sim_dag_data <- function(N, a, b, cond, conf) {
  ...
  # Treatment X
  X <- g[7] * Z + g[8] * W + g[9] * J + g[10] * L + rnorm(N, 0, 0.1)
  X = purrr::rbernoulli(N, p = plogis(X))
...
def sim_dag_data(N, a, b, cond, conf):
  ...
  # Treatment X
  logit_theta = g[6] * Z + g[7] * W + g[8] * J + g[9] * L + rnorm(0, 0.1, N)
...
framework and takiing a Bayesian approoach may in fact be constraining how severe the bias in the
...
1,500 times quickly becomes computationally burdensome. It is possible to side-step this
...
from an hour to about twenty minutes.
:::
...
make_stan_data <- function(sim_data, truth, ...) {
  
  # Predictor Positions
  x_pos <- grep("[J-X]|Z", colnames(sim_data))
...
sim_bayes <- function(stan_model, 
  ...
  stan_data <- make_stan_data(sim_data, truth)
...
# Simulate 1,500 datasets of varying dimensions 
...
Anything in figure @fig-dgp-dag that isn't $X$ is a **nuisance parameter** and generally should not be presented or 
interpreted as if their respective coefficient or marginal effect estimate has some causal meaning. 

Some readers may wonder whether this applies to analyses whose aims are primarily descriptive 
or exploratory in nature whether implicitly or explicitly stated. Since identifying 
relevant confounders is a process that must be informed and justified by theory, the need to adjust for 
potentially confounding factors itself implies causal aims as does any reporting or reliance on arbitrary 
decision rules such as "statistical significance" as a means of establishing whether a relationship exists
[@Wysocki2022]. Indeed, outside of a causal framework p-values have no meaning or valid interpretation.

They 
most certainly are not, as someone recently said to me, "a part of your contribution to existing research." 
In fact, since presenting and interpreting hopelessly confounded coefficient estimates or marginal effects 
encourages those reading your work to do the same, those who persist in committing the table 2 fallacy or 
encourage others to do so are actively harming the pursuit of knowledge in their field by peddling pseudo-science.
